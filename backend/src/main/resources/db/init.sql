...
CREATE TABLE IF NOT EXISTS `feeding_record` (
    ...
    `feeding_type` TINYINT NOT NULL COMMENT '喂养类型: 1-母乳 2-奶粉',
...
CREATE TABLE IF NOT EXISTS `sleep_record` (
    `id` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `baby_id` BIGINT NOT NULL COMMENT '宝宝ID',
    `sleep_type` TINYINT NOT NULL COMMENT '睡眠类型: 1-睡眠 2-夜间睡眠',
    ...
    `next_nap_time` DATETIME COMMENT '下次睡眠预计时间',
...
CREATE TABLE IF NOT EXISTS `feeding_setting` (
    ...
    `default_feeding_type` TINYINT DEFAULT 1 COMMENT '默认喂养类型: 1-母乳 2-奶粉',
    ...
    `reminder_start_time` VARCHAR(10) DEFAULT '06:00:00' COMMENT '提醒时段开始时间',
    `reminder_end_time` VARCHAR(10) DEFAULT '22:00:00' COMMENT '提醒时段结束时间',
...
CREATE TABLE IF NOT EXISTS `sleep_setting` (
    `id` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `baby_id` BIGINT NOT NULL UNIQUE COMMENT '宝宝ID',
    `default_nap_interval` INT DEFAULT 120 COMMENT '默认睡眠间隔（分钟）',
    `default_nap_duration` INT DEFAULT 90 COMMENT '默认睡眠时长（分钟）',
    ...
    `reminder_enabled` TINYINT DEFAULT 1 COMMENT '是否启用提醒: 0-否 1-是',
    `next_nap_reminder_enabled` TINYINT DEFAULT 1 COMMENT '是否启用下次睡眠提醒: 0-否 1-是',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted` TINYINT DEFAULT 0 COMMENT '逻辑删除: 0-未删除 1-已删除'
...
CREATE TABLE IF NOT EXISTS `reminder` (
    ...
    `reminder_type` TINYINT NOT NULL COMMENT '提醒类型: 1-喂奶提醒 2-解冻提醒 3-睡眠提醒 4-哄睡提醒 5-自定义提醒 6-疫苗提醒',
...
CREATE TABLE IF NOT EXISTS `vaccination_record` (
    ...
    `is_free` TINYINT DEFAULT 1 COMMENT '是否免费: 1-国家免费 0-自费',
    `original_vaccine_code` VARCHAR(50) COMMENT '原始疫苗代码（如果选择了替代疫苗）',
    `price` DECIMAL(10,2) COMMENT '疫苗价格（元）',
